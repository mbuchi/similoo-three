...
test('release and package metadata are aligned at 0.13.1', () => {
  assert.equal(pkg.version, '0.13.1');
  assert.equal(lock.version, '0.13.1');
  assert.equal(lock.packages[''].version, '0.13.1');
  assert.match(releases, /export const RELEASES = \[\s*{\s*version: '0\.13\.1'/s);
});
